...
import com.email.emailgen.config.OpenRouterProperties;
...
public class OpenRouterServiceImpl implements AIService {
    ...
    public OpenRouterServiceImpl(WebClient.Builder builder,
                                 OpenRouterProperties openRouterProperties,
                                 AIProperties aiProperties) {
        this.webClient = builder.baseUrl(openRouterProperties.getApiUrl()).build();
    ...
    public AIResponse generate(String prompt) {
        try {
            ...
            String response = webClient.post()
                    .uri(openRouterProperties.getChatEndpoint())
                    ...
                    .onStatus(HttpStatusCode::isError, clientResponse ->
                            clientResponse.createException().flatMap(ex -> {
                                log.warn("OpenRouter request failed with status {}", ex.getStatusCode().value());
                                return Mono.error(toClientException(ex));
                            }))
                    .bodyToMono(String.class)
                    .timeout(openRouterProperties.getTimeout())
